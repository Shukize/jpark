...
(function () {
  ...
  I.registerI18n({
    en: {
      ...
      'pol.updated': 'Last updated: 1 September 2026',
      ...
      'pol.book.pricesP': 'All prices on this website are shown in Thai Baht (THB), per room, per night. Room-only and with-breakfast rates are shown separately, and any extra bed or additional guest charge is added and displayed before you confirm. If you pay online, an online payment fee is added as well — see the section below. Every one of these is itemised on the booking form before you confirm, so the total shown on that form is the amount you are charged.',
      'pol.book.feeH': 'Online payment fee',
      'pol.book.feeP': 'Paying by card or PromptPay QR costs the hotel a percentage of the payment, charged by our payment provider, plus VAT on that percentage. That cost is added to your bill as a separate "online payment fee" line so that the room rate itself stays exactly as advertised. It is shown on the booking form before you confirm, on your confirmation email, and on your receipt. The exact percentage is set by our provider and can change, so the fee is always shown as its own line, in Baht, on the booking form before you confirm — that figure is the one that applies.',
      'pol.book.feeP2': 'There is no online payment fee on a booking settled in person at the front desk, and none on the key-card deposit. If our provider changes its rates, the figure shown on the booking form is always the one that applies. The fee forms part of your payment, so the no-refund terms below cover it in the same way — but if we return a payment to you to correct a billing mistake, the fee is returned with it.',
    ...
    th: {
      ...
      'pol.updated': 'ปรับปรุงล่าสุด: 1 กันยายน 2569',
      ...
      'pol.book.pricesP': 'ราคาทั้งหมดบนเว็บไซต์นี้แสดงเป็นเงินบาท (THB) ต่อห้อง ต่อคืน ราคาเฉพาะห้องพักและราคารวมอาหารเช้าจะแสดงแยกกัน และค่าเตียงเสริมหรือค่าผู้เข้าพักเพิ่มจะถูกคำนวณและแสดงให้เห็นก่อนที่ท่านจะยืนยันการจอง หากท่านชำระเงินออนไลน์ จะมีค่าธรรมเนียมการชำระเงินออนไลน์เพิ่มด้วย (ดูหัวข้อถัดไป) รายการทั้งหมดนี้จะแสดงแยกรายการในแบบฟอร์มการจองก่อนที่ท่านจะยืนยัน ดังนั้นยอดรวมที่แสดงในแบบฟอร์มคือยอดที่ท่านจะถูกเรียกเก็บจริง',
      'pol.book.feeH': 'ค่าธรรมเนียมการชำระเงินออนไลน์',
      'pol.book.feeP': 'การชำระด้วยบัตรเครดิต/เดบิตหรือพร้อมเพย์ทำให้โรงแรมมีต้นทุนเป็นเปอร์เซ็นต์ของยอดชำระ ซึ่งเรียกเก็บโดยผู้ให้บริการรับชำระเงิน บวกภาษีมูลค่าเพิ่มของค่าธรรมเนียมดังกล่าว ต้นทุนนี้จะถูกเพิ่มในใบเรียกเก็บของท่านเป็นรายการ “ค่าธรรมเนียมชำระเงินออนไลน์” แยกต่างหาก เพื่อให้ราคาห้องพักคงเดิมตามที่ประกาศไว้ โดยจะแสดงในแบบฟอร์มการจองก่อนยืนยัน ในอีเมลยืนยันการจอง และในใบเสร็จ อัตราที่แน่นอนกำหนดโดยผู้ให้บริการและอาจเปลี่ยนแปลงได้ ค่าธรรมเนียมนี้จึงแสดงเป็นรายการแยกเป็นจำนวนเงินบาทในแบบฟอร์มการจองก่อนที่ท่านจะยืนยันเสมอ และตัวเลขที่แสดงนั้นคือตัวเลขที่ใช้จริง',
      'pol.book.feeP2': 'การชำระเงินที่เคาน์เตอร์ต้อนรับไม่มีค่าธรรมเนียมนี้ และเงินมัดจำคีย์การ์ดก็ไม่มีเช่นกัน หากผู้ให้บริการเปลี่ยนแปลงอัตราค่าธรรมเนียม ตัวเลขที่แสดงในแบบฟอร์มการจองคือตัวเลขที่ใช้จริงเสมอ ค่าธรรมเนียมนี้ถือเป็นส่วนหนึ่งของยอดที่ท่านชำระ จึงอยู่ภายใต้เงื่อนไขไม่คืนเงินด้านล่างเช่นเดียวกัน แต่หากทางเราคืนเงินให้ท่านเพื่อแก้ไขข้อผิดพลาดในการเรียกเก็บ ค่าธรรมเนียมนี้จะถูกคืนพร้อมกันด้วย',
    ...
    ja: {
      ...
      'pol.updated': '最終更新日：2026年9月1日',
      'pol.navBooking': 'ご予約・キャンセル・返金',
      'pol.navPrivacy': 'プライバシーポリシー',

      'pol.book.title': 'ご予約・キャンセル・返金に関する規定',
      'pol.book.pricesH': '料金と通貨',
      'pol.book.pricesP': '当ウェブサイトの料金はすべてタイバーツ（THB）で、1室1泊あたりの表示です。室料のみの料金と朝食付きの料金は別々に表示され、エキストラベッドや追加のご宿泊者の料金は、ご確定前に加算して表示されます。オンラインでお支払いの場合はオンライン決済手数料も加算されます（次項をご覧ください）。これらはすべてご確定前に予約フォーム上で内訳として表示されますので、フォームに表示された合計金額が実際にご請求される金額です。',
      'pol.book.feeH': 'オンライン決済手数料',
      'pol.book.feeP': 'クレジットカードまたはPromptPay QRでのお支払いには、決済代行会社が課すお支払い金額に対する一定率の手数料と、その手数料に対する消費税がかかります。この費用は「オンライン決済手数料」として別行でご請求に加算され、室料そのものは表示どおりに据え置かれます。予約フォームのご確定前、確認メール、および領収書に表示されます。正確な料率は決済代行会社が定めるもので変更される場合があるため、手数料はご確定前に予約フォーム上でバーツ建ての独立した項目として必ず表示されます。その金額が実際に適用されるものです。',
      'pol.book.feeP2': 'フロントでのお支払いにはこの手数料はかかりません。ルームキーカードのデポジットにもかかりません。決済代行会社が料率を変更した場合も、常に予約フォームに表示されている数値が適用されます。この手数料はお支払い金額の一部ですので、下記の返金不可の規定が同様に適用されます。ただし、ご請求の誤りを訂正するために返金する場合は、手数料もあわせて返金いたします。',
    ...
    'zh-Hans': {
      ...
      'pol.updated': '最后更新：2026年9月1日',
      'pol.navBooking': '预订、取消与退款',
      'pol.navPrivacy': '隐私政策',

      'pol.book.title': '预订、取消与退款政策',
      'pol.book.pricesH': '价格与货币',
      'pol.book.pricesP': '本网站所有价格均以泰铢（THB）显示，按每间客房每晚计算。仅含房费与含早餐的价格分别显示，加床或加人费用会在您确认前计入并显示。若您选择网上付款，还会加收网上支付手续费（详见下节）。以上各项均会在您确认前于预订表单中逐项列出，因此表单上显示的总额即为实际扣款金额。',
      'pol.book.feeH': '网上支付手续费',
      'pol.book.feeP': '使用银行卡或 PromptPay 二维码付款，酒店需向支付服务商支付一定比例的手续费，并就该手续费缴纳增值税。该项成本将以单独的“网上支付手续费”列入您的账单，房费本身则维持公布价格不变。该费用会在预订表单确认前、确认邮件及收据上显示。具体费率由支付服务商设定并可能调整，因此该手续费始终会在您确认前于预订表单中以泰铢单独列示，以该金额为准。',
      'pol.book.feeP2': '在前台当面付款不收取此项手续费，房卡押金亦不收取。若服务商调整费率，以预订表单上显示的数字为准。该手续费属于您所支付款项的一部分，因此同样适用下文的不可退款条款；但若我们为更正计费错误而退款，手续费将一并退还。',
      'pol.book.payH': '支付方式',
    ...
    'zh-Hant': {
      ...
      'pol.updated': '最後更新：2026年9月1日',
      'pol.navBooking': '訂房、取消與退款',
      'pol.navPrivacy': '隱私權政策',

      'pol.book.title': '訂房、取消與退款政策',
      'pol.book.pricesH': '價格與貨幣',
      'pol.book.pricesP': '本網站所有價格均以泰銖（THB）顯示，按每間客房每晚計算。僅含房費與含早餐的價格分別顯示，加床或加人費用會在您確認前計入並顯示。若您選擇網路付款，還會加收網路付款手續費（詳見下節）。以上各項均會在您確認前於訂房表單中逐項列出，因此表單上顯示的總額即為實際扣款金額。',
      'pol.book.feeH': '網路付款手續費',
      'pol.book.feeP': '使用金融卡／信用卡或 PromptPay QR 付款，飯店需向金流服務商支付一定比例的手續費，並就該手續費繳納營業稅。該項成本將以單獨的「網路付款手續費」列入您的帳單，房費本身則維持公告價格不變。該費用會在訂房表單確認前、確認信件及收據上顯示。具體費率由金流服務商設定並可能調整，因此該手續費始終會在您確認前於訂房表單中以泰銖單獨列示，以該金額為準。',
      'pol.book.feeP2': '於櫃檯當面付款不收取此項手續費，房卡押金亦不收取。若服務商調整費率，以訂房表單上顯示的數字為準。該手續費屬於您所支付款項的一部分，因此同樣適用下文的不可退款條款；但若我們為更正計費錯誤而退款，手續費將一併退還。',
      'pol.book.payH': '付款方式',
