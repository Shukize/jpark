...
// Card is a one-step flow: the browser tokenizes the card via Omise.js
// (client-side, so the raw card number never reaches this server — only a
// tokn_... id does), then this creates a charge directly against that
// token. Unlike PromptPay, this resolves SYNCHRONOUSLY: charge.status is
// 'successful' or 'failed' by the time this call returns (barring a 3-D
// Secure "offsite" redirect — see docs/PAYMENTS_SETUP.md's go-live
// checklist for that known limitation).
async function createCardCharge({ amountSatang, currency, token, description, metadata }) {
  return omiseRequest('POST', '/charges', {
    amount: amountSatang,
    currency: currency || 'thb',
    card: token,
